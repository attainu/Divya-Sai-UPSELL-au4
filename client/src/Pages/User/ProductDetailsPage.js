...
class ProductDetailsPage extends Component {
  ...
  addtocart = (cartitem) => {
    ...
    if (cartitem.quantity <= 0) {
      this.setState({
        validationMessage: "Please enter the quantity greater than 0",
      });
    } else if (cartitem.deliverydate == "") {
      this.setState({ validationMessage: "Please choose delivery date" });
    } else if (cartitem.selHour == "") {
      this.setState({
        validationMessage: "Please choose an appropriate time in Hours",
      });
    } else if (cartitem.selMin == "") {
      this.setState({
        validationMessage: "Please choose an appropriate time in Minutes",
      });
    } else {
      ...
      <div>
        ...
        <div class="product-detail-order-wrapper container">
          <div className="product-detail-order-image ">
            <ImageHelper productId={this.props.match.params.id} styles={{}} />
          </div>

          <div
            className="product-detail-order-form"
            style={{ "min-height": "525px" }}
          >
            <form>
              <div className="form-group">
                <label name="product_title">
                  <h1>{this.state.product.product_title}</h1>
                </label>
              ...
              <div className="form-group">
                ...
                <select
                  name="selHour"
                  value={this.state.selHour}
                  onChange={(e) => this.setState({ selHour: e.target.value })}
                  className="form-control"
                  style={{ margin: "10px" }}
                  required
                >
                ...
                <select
                  name="selMin"
                  value={this.state.selMin}
                  onChange={(e) => this.setState({ selMin: e.target.value })}
                  style={{ margin: "10px" }}
                  className="form-control"
                >
                  <option selected="selected" value="">
            ...
            <span
              style={{ color: "red", display: "inline-block", padding: "10px" }}
            >
